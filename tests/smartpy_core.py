...
class PyntellisenseCompletions(QObject):
        ...
        if self.__status:
            ...
            if lexer_name=="python" and not self._runing:
                ...
                try:
                    ...
                    suggestions = self.get_help(complete_helpers)
                    
                    if completions:
                        self.on_load_completions.emit(self._lexer_api, completions)
                    
                    if suggestions[0]:
                        self.on_show_help.emit(self.editor, row, suggestions[0])
    ...
    def get_help(self, complete_helpers):
        help_names = []
        items = []
        for x, suggestion in enumerate(complete_helpers):
            name = suggestion.name
            value = QsciStyledText(name+"  ", 210)
