...
class File(IO):
    ...
    def replace_text_in_files(self, search_text, replace_text, search_dir, case_sensitive=False, search_subdirs=True):
...
class Get:
    ...
    def get_expanded_tab_text(txt:str, tabWidth:int = 8) -> str:
        out=[]
    ...
    def get_code_warnings(self, editor) -> dict:
        return_functions_tuple = (
            ide.tabs_obsolete,
            ide.trailing_whitespace
            )
    ...
    def get_all_lexers_objects_api(self) -> dict:
        return [
            {"name":"python", "lexer":PythonLexer, "icon":self.get_lexer_icon_by_name("python")},
            {"name":"c", "lexer":CLexer, "icon":self.get_lexer_icon_by_name("c")},
            {"name":"c++", "lexer":CPPLexer, "icon":self.get_lexer_icon_by_name("c++")},
            {"name":"css", "lexer":CSSLexer, "icon":self.get_lexer_icon_by_name("css")},
            {"name":"html", "lexer":HTMLLexer, "icon":self.get_lexer_icon_by_name("html")},
            {"name":"javaScript", "lexer":JavaScriptLexer, "icon":self.get_lexer_icon_by_name("javascript")},
            {"name":"json", "lexer":JSONLexer, "icon":self.get_lexer_icon_by_name("json")},
            {"name":"text", "lexer":NoneLexer, "icon":self.get_lexer_icon_by_name("text")}
        ]
    ...
    def get_icon_api(self):
        api_file = pathlib.Path(f"{BASE_PATH}{SYS_SEP}smartcode{SYS_SEP}icons{SYS_SEP}{config.get_icons_package()}{SYS_SEP}main.json")
        if api_file.is_file() and api_file.exists():
            return ijson.load(api_file)
        return None

    def get_app_icon(self) -> object:
        return self.get_qicon(f"smartcode{SYS_SEP}icons{SYS_SEP}icons{SYS_SEP}logo-min.svg")
    ...
    def get_application_icons(self, area:str="-") -> dict:
        ...
        if area == "index":
            return {
                "logo":f"smartcode{SYS_SEP}icons{SYS_SEP}icons{SYS_SEP}logo.svg",
                "python":f"smartcode{SYS_SEP}icons{SYS_SEP}icons{SYS_SEP}python.svg"
            }
