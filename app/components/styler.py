...
windows_style=config.get_window_style()

def apply_base_theme(config, qapp):
    qapp.setStyle(config.get_qt_theme())
...
def beautify(qapp, app=None):
    ...
    if config.get_palette() in {"dark","black",0,"night"}:
        dark(qapp)
    
    elif config.get_palette() in {"light","white",1,"day"}:
        light(qapp)
    
    else:
        pass
    
    apply_base_theme(config, qapp)
