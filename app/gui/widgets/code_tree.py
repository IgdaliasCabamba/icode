...
class CodeTreeCore(QObject):
    ...
    def get_code_parse(self, code, editor):
        try:
            python_tree=getfn.get_python_node_tree(code)
                ...
                for element in python_tree:
                    ...
                    if element.type == "class":
                        class_row=IStandardItem(
                            ...
                            {"line":element.line_number, "name":element.name, "editor":editor},
                            1
                            )
                        ...
                        for child_element in element.children:
                            child_tree_item = IStandardItem(
                                ...
                                {"line":child_element.line_number, "name":child_element.name, "editor":editor},
                                2
                                )
...
class CodeTree(QTreeView):
    ...
    def prepare_tree(self):
        ...
        self.doc = IStandardItem(self.icons.get_icon("file"), f'DOCUMENT:{file_name}', f"{file}", None, 0)
        self.doc.setCheckable(False)
        self.model.appendRow(self.doc)
        
        self.modules = IStandardItem(self.icons.get_icon("import"), 'MODULES', None, None, 0)
        self.modules.setCheckable(False)
        self.model.appendRow(self.modules)

        self.classes = IStandardItem(self.icons.get_icon("class"), 'CLASSES', None, None, 0)
        self.classes.setCheckable(False)
        self.model.appendRow(self.classes)

        self.functions = IStandardItem(self.icons.get_icon("function"), 'FUNCTIONS', None, None, 0)
        self.functions.setCheckable(False)
        self.model.appendRow(self.functions)

        self.variables = IStandardItem(self.icons.get_icon("global_variable"), 'VARIABLES', None, None, 0)
