...
class DataManager(QSettings):
    def __init__(self, file_with_path: str, format=QSettings.Format.IniFormat) -> None:
        super().__init__(file_with_path, format)
        self.file_path = os.path.join(os.environ["SMARTCODE_SRC_PATH"], "data", file_with_path)
...
class CacheManager(QSettings):
    def __init__(
        self,
        file_with_path: str,
        format: object = QSettings.Format.IniFormat,
        optimization: int = 3,
    ):
        super().__init__(file_with_path, format)
