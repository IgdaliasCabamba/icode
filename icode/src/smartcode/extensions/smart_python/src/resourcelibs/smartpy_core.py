    ...
    def analyze_conditions(self, material:str) -> str:
        ...
        if len(condition) > 3:
            if condition[3] == "True" or condition[3] == "False":
                return QsciStyledText(f"> Anti-pattern: '{condition[2]}'\n When comparing a variable to boolean, you should use the form\n> '{condition[0]} {condition[1]} is {condition[3]}' or simply '{condition[0]} {condition[1]}'", 200)
            
            if condition[3] == "None":
                return QsciStyledText(f"> Anti-pattern: '{condition[2]}'\n Comparisons to the singleton objects, like True, False, and None\nshould be done with identity, not equality. Use “is” or “is not”\n> SUGGESTION: '{condition[0]} {condition[1]} is {condition[3]}' or '{condition[0]} is not {condition[1]}'", 200)
...
class PyntellisenseCompletions(QObject):
    ...
    def get_urls(self):
        ...
        if server is not None:
            ...
            self.urls["complete-search"] = f"http://{server.host}:{server.port}/complete_search"
            self.__status = True
        ...
        if self.__status:
            ...
            if lexer_name=="python" and not self._runing:
                ...
                try:
                    ...
                    complete_response = requests.get(self.urls["complete"], data=json.dumps(complete_request_data))
                    complete_help_response = requests.get(self.urls["complete-search"], data = json.dumps(complete_help_request_data))
                    ...
                    if row != self.last_row:                        
                        ...
                        statements = sorted({node.id for node in ast.walk(root) if isinstance(node, ast.Name)})
                    ...
                    if dead_statement:
                        self.on_remove_dead_completion.emit(self._lexer_api, dead_statement)
