class ImportStatement(object):
    ...
    def _set_import_info(self, new_import):
        if (not self.readonly and new_import is not None
    ...
    def is_changed(self):
        return self._is_changed or (self.new_start is not None
                                    or self.new_start != self.start_line)
...
class FromImport(ImportInfo):
    ...
    def get_imported_resource(self, context):
        ...
        """
        if self.level == 0:
            return context.project.find_module(self.module_name,
    ...
    def get_imported_module(self, context):
        ...
        if self.level == 0:
            ...
            return context.project.get_relative_module(self.module_name,
                                                       context.folder,
                                                       self.level)
    ...
    def is_star_import(self):
        return len(
            self.names_and_aliases) > 0 and self.names_and_aliases[0][0] == "*"
