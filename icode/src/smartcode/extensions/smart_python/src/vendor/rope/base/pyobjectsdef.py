...
class PyFunction(pyobjects.PyFunction):
    ...
    def __init__(self, pycore, ast_node, parent):
        ...
        rope.base.pyobjects.PyDefinedObject.__init__(self, pycore, ast_node,
                                                     parent)
        self.arguments = self.ast_node.args
        self.parameter_pyobjects = pynames._Inferred(
            self._infer_parameters,
            self.get_module()._get_concluded_data())
...
class PyComprehension(pyobjects.PyComprehension):
    ...
    def _create_scope(self):
        return rope.base.pyscopes.ComprehensionScope(self.pycore, self,
                                                     _ComprehensionVisitor)
    ...
    def _get_bases(self):
        ...
        for base_name in self.ast_node.bases:
            base = rope.base.evaluate.eval_node(self.parent.get_scope(),
                                                base_name)
            if (base is not None and base.get_object().get_type()
...
class PyPackage(pyobjects.PyPackage):
    ...
    def _get_init_dot_py(self):
        if self.resource is not None and self.resource.has_child(
                "__init__.py"):
...
class _ExpressionVisitor(object):
    ...
    def _GeneratorExp(self, node):
        list_comp = PyComprehension(self.scope_visitor.pycore, node,
...
class _ScopeVisitor(_ExpressionVisitor):
    ...
    def _update_evaluated(self,
        ...
        if isinstance(targets, str):
            assignment = pynames.AssignmentValue(assigned, [], evaluation,
                                                 eval_type)
        ...
        if len(node.args.args) > 0:
            ...
            if isinstance(first, pycompat.ast_arg_type):
                new_visitor = _ClassInitVisitor(
                    self, pycompat.get_ast_arg_arg(first))
...
class _ClassInitVisitor(_AssignVisitor):
    ...
    def _Attribute(self, node):
        ...
        if isinstance(node.value,
                      ast.Name) and node.value.id == self.self_name:
