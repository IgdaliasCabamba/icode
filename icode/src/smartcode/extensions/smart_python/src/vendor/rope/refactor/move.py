...
class MoveMethod(object):
    ...
    def _get_scope_indents(self, pyobject):
        ...
        return sourceutils.get_indents(
            pymodule.lines,
            pyobject.get_scope().get_start()) + sourceutils.get_indent(
                self.project)
...
class MoveGlobal(object):
    ...
    def _source_module_changes(self, dest):
        ...
        renamer = ModuleSkipRenamer(occurrence_finder, self.source, handle,
                                    start, end)
        ...
        if handle.occurred:
            pymodule = libutils.get_string_module(self.project, source,
                                                  self.source)
            # Adding new import
            source, imported = importutils.add_import(self.project, pymodule,
                                                      self._new_modname(dest),
                                                      self.old_name)
            source = source.replace(placeholder, imported)
    ...
    def _get_moving_element_with_imports(self):
        return moving_code_with_imports(self.project, self.source,
                                        self._get_moving_element())

    def _get_module_with_imports(self, source_code, resource):
        pymodule = libutils.get_string_module(self.project, source_code,
                                              resource)
    ...
    def _get_moving_region(self):
        ...
        while start_line > 1 and lines.get_line(start_line -
                                                1).startswith("#"):
            start_line -= 1
        start = lines.get_line_start(start_line)

        while end_line < lines.length() and lines.get_line(end_line +
                                                           1).strip() == "":
            end_line += 1
...
class MoveModule(object):
    ...
    def _change_occurrences_in_module(self,
                                      dest,
                                      pymodule=None,
                                      resource=None):
        ...
        should_import = self.tools.occurs_in_module(pymodule=pymodule,
                                                    resource=resource,
                                                    imports=False)
        pymodule = self.tools.new_pymodule(pymodule, source)
    ...
    def _change_import_statements(self, dest, new_name, module_imports):
        ...
        for import_stmt in module_imports.imports:
            if not any(name_and_alias[0] == self.old_name for name_and_alias in
                       import_stmt.import_info.names_and_aliases) and not any(
            ...
            if (not import_stmt.import_info.is_empty()
    ...
    def _handle_moving_in_from_import_stmt(self, dest, import_stmt,
        ...
        changed = False
        context = importutils.importinfo.ImportContext(self.project, None)
        if import_stmt.import_info.get_imported_resource(
...
class _MoveTools(object):
    ...
    def remove_old_imports(self, pymodule):
        ...
        class CanSelect(object):
            ...
            def __call__(self, name):
                try:
                    if (name == self.old_name and pymodule[name].get_object()
    ...
    def new_pymodule(self, pymodule, source):
        if source is not None:
            return libutils.get_string_module(self.project, source,
                                              pymodule.get_resource())
        return pymodule
