...
class History(object):
    ...
    def _search_for_change_contents(self, change_list, file):
        for change_ in reversed(change_list):
            if isinstance(change_, change.ChangeSet):
                result = self._search_for_change_contents(
                    change_.changes, file)
                if result is not None:
                    return result
            if isinstance(change_,
                          change.ChangeContents) and change_.resource == file:
    ...
    def write(self):
        if self.save:
            ...
            self.project.data_files.write_data("history",
                                               data,
                                               compress=self.compress)
    ...
    def __str__(self):
        return "History holds %s changes in memory" % (len(self.undo_list) +
                                                       len(self.redo_list))
