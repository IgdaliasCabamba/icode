...
def module_with_reason(name: str,
                       config: Config = DEFAULT_CONFIG) -> Tuple[str, str]:
    """Returns the section placement for the given module name alongside the reasoning."""
    return (_forced_separate(name, config) or _local(name, config)
            or _known_pattern(name, config) or _src_path(name, config)
            or (config.default_section,
                "Default option in Config or universal default."))
...
def _known_pattern(name: str, config: Config) -> Optional[Tuple[str, str]]:
    parts = name.split(".")
    module_names_to_check = (".".join(parts[:first_k])
    ...
    for module_name_to_check in module_names_to_check:
        for pattern, placement in config.known_patterns:
            if placement in config.sections and pattern.match(
                ...
                return (placement,
                        f"Matched configured known pattern {pattern}")
...
def _src_path(
    ...
    for src_path in src_paths:
        ...
        if (not prefix and not module_path.is_dir()
                and src_path.name == root_module_name):
            module_path = src_path.resolve()
        if nested_module and (namespace in config.namespace_packages or
                              (config.auto_identify_namespace_packages
                               and _is_namespace_package(
                                   module_path, config.supported_extensions))):
            return _src_path(nested_module[0], config, (module_path, ),
                             new_prefix)
...
def _is_module(path: Path) -> bool:
    return (exists_case_sensitive(str(path.with_suffix(".py"))) or any(
...
def _is_namespace_package(path: Path, src_extensions: FrozenSet[str]) -> bool:
    ...
    if not init_file.exists():
        filenames = [
            filepath for filepath in path.iterdir()
        ...
        with init_file.open("rb") as open_init_file:
            ...
            if (b"__import__('pkg_resources').declare_namespace(__name__)"
                    not in file_start and
...
def _src_path_is_module(src_path: Path, module_name: str) -> bool:
    return (module_name == src_path.name and src_path.is_dir()
