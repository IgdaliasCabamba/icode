...
def sorted_definitions(defs):
    ...
    return sorted(
        defs,
        key=lambda x:
        (str(x.module_path or ""), x.line or 0, x.column or 0, x.name),
    )
...
def _get_signature_details_from_error_node(node, additional_children,
                                           position):
    for index, element in reversed(list(enumerate(node.children))):
        ...
        if element == "(" and element.end_pos <= position and index > 0:
            ...
            if name.type == "name" or name.parent.type in ("trailer", "atom"):
                return CallDetails(element, children + additional_children,
                                   position)
    ...
    while node is not None:
        ...
        if (node.type == "trailer" and node.children[0] == "("
            ...
            if not (leaf is node.children[-1] and position >= leaf.end_pos):
                ...
                return CallDetails(
                    node.children[0]
                    if node.type == "trailer" else node.children[2],
                    node.children,
                    position,
                )
