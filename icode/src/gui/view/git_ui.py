...
from smartlibs.qtmd import HeaderPushButton, HeaderLabel, InputHistory
from .igui import IStandardItem
import re
...
class StatusTree(QTreeView):
    ...
    def build_tree(self, repository):
        try:
            ...
            self.unmodified = IStandardItem(
                self.icons.get_icon("unmodified"),
                "UNMODIFIED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.modified = IStandardItem(
                self.icons.get_icon("modified"),
                "MODIFIED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.added = IStandardItem(
                self.icons.get_icon("added"),
                "ADDED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.deleted = IStandardItem(
                self.icons.get_icon("deleted"),
                "DELETED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.renamed = IStandardItem(
                self.icons.get_icon("renamed"),
                "RENAMED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.copied = IStandardItem(
                self.icons.get_icon("copied"),
                "COPIED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.updated_but_unmerged = IStandardItem(
                self.icons.get_icon("updated_unmerged"),
                "UPDATED BUT UNMERGED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.untracked = IStandardItem(
                self.icons.get_icon("untracked"),
                "UNTRACKED " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
            ...
            self.ignored = IStandardItem(
                self.icons.get_icon("ignored"),
                "IGNORED FILES " + get_unicon("dev", "git"),
                None,
                None,
                0,
            )
...
class IGit(QFrame):
    ...
    def update_headers(self, total, _, __):
        self.lbl_total.setText(
            f"<p><span style='font-size:15pt'>{get_unicon('dev', 'git')}</span> {str(total)}</p>"
        )
